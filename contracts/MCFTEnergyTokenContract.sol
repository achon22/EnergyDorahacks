...
contract MCFTEnergyTokenContract is AccessControl, ERC1178 {
  ...
  uint256 minCount;
  string hello; // delete after testing
  struct Transactor {
    address actor;
    uint256 amount;
  }
  ...
  constructor () public {
    ...
    tokenCount = 0;
    minCount = 1000;
    minTokenPrice = 200000000000000; // around 0.086 cents per token
    hello = "howdy";
  }
  ...
  */
  ...
  function registerCompany(string companyName, uint256 count) public payable returns (bool){
    require(msg.value >= count * minTokenPrice && count >= minCount);
    ownerToClassToBalance[msg.sender][currentClass] = count;
    classNames[count] = companyName;
    currentClass += 1;
    return true;
  }
  ...
  function test(uint256[] array) public{
    hello = 'array params work ;)';
  }
